...
| **`filings.recent` IS A ROLLING WINDOW OF ROUGHLY A THOUSAND FILINGS, NOT THE INDEX**, so a heavy Form 4 filer's own history can fall off it with no error and no log line. Whether that is a defect depends on the window the component needs, not on the read. | CRWV's recent page reaches back only **13 months** because 460 of its 1001 filings are Form 4s, so its 10-Q for period 2025-03-31 was unread. That filing carries the longest lag of its five, and `earnings_calendar` published `Thu 12 Nov` to Discord while `snapshot.json` published `2026-11-13` off the same index — the same two components the shared `filing_cadence` was extracted to reconcile, because **the merge shared the rule and not the corpus**. `probe_cadence_corpus.py` feeds both presentations the same list and so could never see it. The exposure rule is what generalises: a company moves only if its selected pool is under `LAG_SAMPLE` **and** it has a second page, since `pool[:LAG_SAMPLE]` truncates positionally — SLNH gains 51 periodic filings and publishes the same row. **All six readers of `recent` were surveyed on 2026-09-12 and the other five are correctly scoped**: `filer_regime` and `audit_8k_items` already page, and `press_monitor` (30d), `weekly_digest` (4 weeks) and `comment_letters` (180d) need less than the window spans. [earnings.md](docs/earnings.md) |

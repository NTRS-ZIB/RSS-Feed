...
existing one far more often than they create one. Adding that structure early
costs a reader a level of navigation to save them nothing, and the clusters
sitting there visibly is not a reason to build them.

**The table reached eighteen rows on 2026-08-13, so grouping is now in range
and is the next structural change here.** The largest cluster is the obvious
place to start: four rows are now some version of *a source that looks healthy
and is not*, and the newest of them arrived as a variant of an existing row
rather than as a new hazard.

...
## A correction: the overlap figure I gave you was measured wrong

I said 6 of 10 feed items had a newsroom twin. **The live run says 2 of 10.**

Six of ten was the feed against the **276-card archive** at
`/all-news/announcements`. The scraper does not read the archive — it reads the
newsroom page, which carries **four** announcements. Both numbers are true and
they answer different questions.

That is the third time on this repo that a number about an adjacent population
got used to answer the one in front of it, and this time I produced it while
writing up a warning about exactly that. It is corrected in the docstring and
in `watchlist.py` rather than only here.

**It does not change the decision** — if anything it strengthens it. The
duplicate problem is smaller than I claimed and the feed's unique items are
unchanged, so the reframe holds harder: the feed's value and the dedupe barely
intersect.
